...
test.describe('Test empty basket (storage debug)', () => {
  ...
  test('Add 4 product items(unsame types) without discount in basket', async ({ page }) => {    
    const itemsToAdd: number = 4;  
    const productsInBasket: Array<string> = []; 
    
    const productsPage = new ProductsPage(page);    
    await productsPage.addNumberOfUnsameTypeItemsNoDiscountTypeInBasket(itemsToAdd, productsInBasket);
    ...
    await basketModal.checkBasketModalItems();
